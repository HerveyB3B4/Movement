 ...
 *                  ------------------------------------
 *                  模块管脚              单片机管脚
 *                  SCL                 查看 zf_device_tft180.h 中
 *TFT180_SCL_PIN 宏定义 SDA                 查看 zf_device_tft180.h 中
 *TFT180_SDA_PIN 宏定义 RES                 查看 zf_device_tft180.h 中
 *TFT180_RES_PIN 宏定义 DC                  查看 zf_device_tft180.h 中
 *TFT180_DC_PIN 宏定义 CS                  查看 zf_device_tft180.h 中
 *TFT180_CS_PIN 宏定义 BL                  查看 zf_device_tft180.h 中
 *TFT180_BL_PIN 宏定义 VCC                 3.3V电源 GND                 电源地
 *                  最大分辨率160*128
 *                  ------------------------------------
 ********************************************************************************************************************/
...
#define TFT180_RST(x) \
    ((x) ? (gpio_high(TFT180_RES_PIN)) : (gpio_low(TFT180_RES_PIN)))
#define TFT180_CS(x) \
    ((x) ? (gpio_high(TFT180_CS_PIN)) : (gpio_low(TFT180_CS_PIN)))
#define TFT180_BLK(x) \
    ((x) ? (gpio_high(TFT180_BL_PIN)) : (gpio_low(TFT180_BL_PIN)))
//=================================================定义 TFT180
// 基本配置================================================
...
// 参数说明     height          图像实际高度
// 返回参数     void
// 使用示例     tft180_displayimage7725(ov7725_image_binary[0], OV7725_W,
// OV7725_H); 备注信息 拓展的一键显示函数，默认无缩放，从屏幕坐标起始点开始显示
//-------------------------------------------------------------------------------------------------------------------
#define tft180_displayimage7725(p, width, height) \
    (tft180_show_binary_image(0, 0, (p), OV7725_W, OV7725_H, (width), (height)))
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     TFT180 显示总钻风图像
// 参数说明     p               图像数组指针
// 参数说明     width           图像实际宽度
...
//-------------------------------------------------------------------------------------------------------------------
#define tft180_displayimage03x(p, width, height)                      \
    (tft180_show_gray_image(0, 0, (p), MT9V03X_W, MT9V03X_H, (width), \
                            (height), 0))
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     TFT180 显示凌瞳图像
// 参数说明     p               图像数组指针
...
// 备注信息     拓展的一键显示函数，默认无缩放，从屏幕坐标起始点开始显示
//-------------------------------------------------------------------------------------------------------------------
#define tft180_displayimage8660(p, width, height)                       \
    (tft180_show_rgb565_image(0, 0, (p), SCC8660_W, SCC8660_H, (width), \
                              (height), 1))
//=================================================声明 TFT180
// 扩展函数================================================
