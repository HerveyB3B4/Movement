 ...
 *                  ------------------------------------
 *                  模块管脚             单片机管脚
 *                  RX                 查看 zf_device_wireless_uart.h 中
 *WIRELESS_UART_RX_PINx 宏定义 TX                 查看 zf_device_wireless_uart.h
 *中 WIRELESS_UART_TX_PINx 宏定义 RTS                查看
 *zf_device_wireless_uart.h 中 WIRELESS_UART_RTS_PINx 宏定义
 *                  VCC                3.3V电源
 *                  GND                电源地
 *                  其余引脚悬空
 *                  ------------------------------------
...
uint32 wireless_uart_send_buffer(const uint8* buff, uint32 len) {
    ...
    while (0 != len) {
        if (!gpio_get_level(
                WIRELESS_UART_RTS_PIN))  // 如果RTS为低电平 则继续发送数据
        {
        ...
        {
            if (WIRELESS_UART_TIMEOUT_COUNT <=
                (++time_count))  // 超出了最大等待时间
            {
                break;  // 退出发送
            }
            system_delay_ms(1);
...
uint32 wireless_uart_send_string(const char* str) {
    ...
    while (0 != len) {
                ...
                WIRELESS_UART_RTS_PIN))  // 如果RTS为低电平 则继续发送数据
        {
            if (30 <= len)  // 数据分 30byte 每包发送
            {
                uart_write_buffer(WIRELESS_UART_INDEX, (const uint8*)str,
                                  30);  // 发送数据
                str += 30;              // 地址偏移
                len -= 30;              // 数量
                time_count = 0;
...
// 返回参数     void
// 使用示例     wireless_uart_send_image(&mt9v03x_image[0][0],
// MT9V03X_IMAGE_SIZE); 备注信息
//-------------------------------------------------------------------------------------------------------------------
...
void wireless_uart_callback(void) {
    ...
    if (WIRELESS_UART_AUTO_BAUD_RATE_START == wireless_auto_baud_flag &&
        3 == fifo_used(&wireless_uart_fifo)) {
        ...
        fifo_read_buffer(&wireless_uart_fifo, (uint8*)wireless_auto_baud_data,
                         (uint32*)&wireless_auto_baud_count,
                         FIFO_READ_AND_CLEAN);
    }
#endif
}
...
uint8 wireless_uart_init(void) {
    ...
    fifo_init(&wireless_uart_fifo, FIFO_DATA_8BIT, wireless_uart_buffer,
    ...
    // 本函数使用的波特率为115200 为无线转串口模块的默认波特率
    // 如需其他波特率请自行配置模块并修改串口的波特率
    uart_init(WIRELESS_UART_INDEX, WIRELESS_UART_BUAD_RATE,
              WIRELESS_UART_RX_PIN, WIRELESS_UART_TX_PIN);  // 初始化串口
    ...
    gpio_init(WIRELESS_UART_RTS_PIN, GPO, rts_init_status,
              GPO_PUSH_PULL);  // 初始化流控引脚
    ...
    gpio_set_level(WIRELESS_UART_RTS_PIN,
                   !rts_init_status);  // RTS引脚拉高，进入自动波特率模式
    ...
    do {
        ...
        uart_write_byte(
            WIRELESS_UART_INDEX,
            wireless_auto_baud_data[0]);  // 发送特定数据 用于模块自动判断波特率
        uart_write_byte(
            WIRELESS_UART_INDEX,
            wireless_auto_baud_data[1]);  // 发送特定数据 用于模块自动判断波特率
        uart_write_byte(
            WIRELESS_UART_INDEX,
            wireless_auto_baud_data[2]);  // 发送特定数据 用于模块自动判断波特率
        system_delay_ms(20);

        if (WIRELESS_UART_AUTO_BAUD_RATE_GET_ACK !=
            wireless_auto_baud_flag)  // 检验自动波特率是否完成
        {
        ...
        gpio_init(WIRELESS_UART_RTS_PIN, GPI, 0,
                  GPI_PULL_UP);  // 初始化流控引脚
